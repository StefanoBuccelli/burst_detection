...
%% load detection from cochlea
cochlea_res= import_results_cochlea(['D:\Capocaccia\old_results_basal_drug\results\basal\original\basal_ch' num2str(ch_num) '.csv']);
...
for curr_time=2:length(cochlea_res.Time)
   ...
   if falling_edge==1 
       if (cochlea_res.class(curr_time)-cochlea_res.class(curr_time-1))==1
           ...
           start_stop_cochlea(new_rising,1)=cochlea_res.Time_stretch(curr_time)-0.2;%subtract 100ms from start assuming original timing is the central
%            start_stop_cochlea(new_rising,1)=cochlea_res.Time_stretch(curr_time);
        ...
        if (cochlea_res.class(curr_time)-cochlea_res.class(curr_time-1))==(-1)
           falling_edge=1;
           start_stop_cochlea(new_rising,2)=cochlea_res.Time_stretch(curr_time)-0.2;%subtract 100ms from end assuming original timing is the central
...
subplot(3,1,1)
n_bursts=[height(resultCMA) height(resultHenning) height(resultPS) height(resultRS) height(resultLogISI) height(resultMI) height(resultHSMM) size(burst_new_s,1) size(start_stop_cochlea,1) size(visual_inspection,1)];
...
mean_vals=[mean(resultCMA.durn) mean(resultHenning.durn) mean(resultPS.durn) mean(resultRS.durn) mean(resultLogISI.durn) mean(resultMI.durn) mean(resultHSMM.durn) mean(burst_new_dur) mean(start_stop_cochlea_dur) mean(start_stop_vi_duration)];
std_vals=[std(resultCMA.durn) std(resultHenning.durn) std(resultPS.durn) std(resultRS.durn) std(resultLogISI.durn) std(resultMI.durn) std(resultHSMM.durn) std(burst_new_dur) std(start_stop_cochlea_dur) std(start_stop_vi_duration)];
...
savefig(burst_numb_dur,'Figures\burst_numb_dur')

%% correlation between burst events, first: create 0-1 signals with burst events
all_signals=[signal_vi; signal_nas ;signal_ch; signal_hsmm; signal_mi; signal_logisi; signal_rs; signal_ps; signal_henning; signal_cma]';
...
savefig(pc_imagesc,'Figures\PC_imagesc')
...
xlabel('Time [s]')
savefig(cc_overlap,'Figures\cc_overlap')
...
xlabel('Time [s]')
linkaxes(h_b,'x')
savefig(figure_burst_raw_hpf,'Figures\figure_burst_raw_hpf')
